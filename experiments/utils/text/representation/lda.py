...
class LDA:
    def __init__(self, args, useSklearn=False, tokenizer=None, process=None): #sklearn not configured
    ...
    def generate_embedding(self, data, useDataCampPrePrcoessing=True, returnarray=True):
        ...
        # generate LSA model
        self.model = LdaModel(doc_term_matrix, num_topics=self.args.n_topics, id2word=dictionary)
    ...
    def preprocess_data2(self, doc_set):
        """
        Input  : document list
        Purpose: preprocess text (tokenize, removing stopwords, and stemming)
        Output : preprocessed text
    ...
    def compute_coherence_values(self, dictionary, doc_term_matrix, doc_clean, stop, start=2, step=3):
        ...
        for num_topics in range(start, stop, step):
            # generate LSA model
            model = LdaModel(doc_term_matrix, num_topics=num_topics, id2word = dictionary)  # train model
