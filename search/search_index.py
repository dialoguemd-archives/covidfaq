...
from utils import DocText, SectionText, ElasticResults
...
def query_question(es, q):
    ...
    topk_doc = 1
    topk_sec = 1
    ## Todo: convert dict to objects in utils.py
    def helper(docindex, secindex):
        res_doc = search_document_index(es, docindex, q, topk_doc)["hits"]["hits"]
        if len(res_doc):
            res_doc_dict = res_doc[0]["_source"]
            res_doc_obj = None
            res_doc_txt = json.dumps(res_doc_dict)
        else:
            res_doc_dict =None
            res_doc_obj = None
            res_doc_txt = "None"
        ...
        if len(res_sec):
            res_sec_dict = res_sec[0]["_source"]
            res_sec_dict = None
            res_sec_txt = json.dumps(res_sec_dict)
        else:
            res_sec_dict = None
            res_sec_dict = None
            res_sec_txt = "None"
