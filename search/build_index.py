...
def fill_index(es, files, docindex, secindex):
    ...
    for i in tqdm(files):
        with open(i, 'r', encoding='latin1') as f:
            ...
            for sec in t:
                ...
                c += 1
    print('{} and {}: inserted/Updated {} documents and {} sections'.format(
        docindex, secindex,
        c_d,
        c_s
    ))
    ...
    print('{} and {}: total {} documents and {} sections'.format(
        docindex, secindex,
...
if __name__ == '__main__':
    ...
    ## Load files
    jsonfiles = [f for f in listdir(scrape_path) if isfile(join(scrape_path, f)) if '.json' in f \
                 and 'faq' not in f and 'mainpage' not in f]
