...
flags.DEFINE_string(
    "predict_file", "test_process7.jsonl",
...
def get_tokinezed_input_from_txt(new_output):
  from spacy.lang.en import English
  required_tags_encoded = ['xH1x', 'xH2x', 'xH3x', 'xH4x', 'xH5x', 'xH6x', 'xPx', 'xLIx', 'xOLx',
                         'xULx',
                'xxH1x', 'xxH2x', 'xxH3x', 'xxH4x', 'xxH5x', 'xxH6x', 'xxPx', 'xxLIx', 'xxOLx',
                 'xxULx'
                ]
  nlp = English()
  parsed_sentence = nlp(new_output)
  token_spaces = [(token.text,token.idx, token.i) for token in parsed_sentence]
  #d = [{"start_byte": 92, "token": "Trade", "html_token": false, "end_byte": 97}]
  #{"start_token": 44, "top_level": True, "start_byte": 43178, "end_token": 161, "end_byte": 44666}
  ...
  end_token = 0
  end_byte = 0
  for (token, start, start_id) in token_spaces:
      is_html = False
      if token in required_tags_encoded:
          is_html = True
      if token in required_tags_encoded:
          if token == 'xPx':
              start_token = True
          ...
          if token[:2] == 'xx':
              token = token.replace('xx','</')

          elif token[:1] == 'x':
              token =  token.replace('x','<', 1)
          token = token.replace('x','>')
      if end_la == True:
          end_byte = start + len(token)
          la = {
              "start_token": start_token,
              "top_level": True,
              "start_byte": start_byte,
              "end_token": start_id + 1,
              "end_byte": end_byte
          }
          la_cndidates.append(la)
          start_la = False
          start_token = 0
          start_byte = 0
          end_la = False
      ...
      token_tuple_list.append(d)

      url = ""
      output  =  text
      #output = text
      doc_title = ""
      #print("token tuple list is", token_tuple_list)
  return token_tuple_list, la_cndidates, url, doc_title, output

def get_tokinezed_input(url):
    ...
    required_ids = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'li', 'ol', 'ul']
    required_tags = ['<H1>', '<H2>', '<H3>', '<H4>', '<H5>', '<H6>', '<P>', '<LI>', '<OL>', '<UL>',
                    '<\H1>', '<\H2>', '<\H3>', '<\H4>', '<\H5>', '<\H6>', '<\P>', '<\LI>', '<\OL>', 
                     '<\\UL>'
                    ]

    for t in text:
        tag_name = t.name.upper()
        tag_text = t.get_text()
        if tag_text!="\n":
          output += '<'+ tag_name +'> ' + tag_text + ' <\\'+ tag_name + '> '
    ...
    end_token = 0
    end_byte = 0
    #print("length if oken space", token_spaces)
    ...
    #print(la)
    #print("token tuple list is ", token_tuple_list)
    ans = token_tuple_list[la['start_token']:la['end_token']]
    out_para= ''
...
def get_tokens_from_question(question):
    ...
    q_token_list = []
    for token in tokens:
        q_token_list.append(token.text)
    q_token_list
    return question, q_token_list


def get_input_json(token_tuple_list, la_cndidates, url, doc_title, output, question, q_token_list):

    j = {"long_answer_candidates": la_cndidates, 
     "document_tokens": token_tuple_list,
     "question_text": question, 
     "example_id": 5225754983651766092, 
     "document_url": "https://en.wikipedia.org//w/index.php?title=Trade_winds&amp;oldid=817251427", 
     "document_title": doc_title, 
     "annotations": [], 
     "document_html": output,
     "question_tokens":q_token_list
    }
    with open("test_process7.jsonl", "w") as f:
      json.dump(j, f)
    return j
...
def create_example_from_jsonl(line):
  ...
  print("XXXXXXXXXXXXXXX e is ", e["long_answer_candidates"])
...
def make_nq_answer(contexts, answer):
  ...
  #print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX______-----------_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
  #print(input_text)
...
def read_candidates_from_one_split(input_path):
  ...
  candidates_dict = {}
  with tf.gfile.Open(input_path, "rb") as input_file:
...
def get_answer_text(nq_pred_dict, j):
    start_tt = nq_pred_dict[5225754983651766092]["long_answer"]["start_token"] 

    end_tt = nq_pred_dict[5225754983651766092]["long_answer"]["end_token"]

    #start_ttt = nq_pred_dict[5225754983651766092]["short_answers"]["start_token"] 

    #end_ttt = nq_pred_dict[5225754983651766092]["short_answers"]["end_token"]
    token_tuple_list = j["document_tokens"]
    doc_toc = token_tuple_list
    #print("XXXLA CANDIDATE",la_cndidates)
    print("Long answer: ")
    ans = doc_toc[start_tt:end_tt]
    long_ans= ''
    for a in ans:
    #print(ans)
        if a["html_token"] == False:
            long_ans += a["token"] + " "

    #if doc_toc[start_tt]["html_token"] == False:
        #i = start_tt - 1
        #while doc_toc[i]["token"] != '<P>' and i>0:
        #    if doc_toc[i]["html_token"] == False:
        #        long_ans = doc_toc[i]["token"] + " " + long_ans
        #    i -= 1        
    #if doc_toc[end_tt]["html_token"] == False:
        #i = end_tt + 1
        #while doc_toc[i]["token"] != '</P>' and i<len(doc_toc):
        #    if doc_toc[i]["html_token"] == False:
        #        long_ans += " " +  doc_toc[i]["token"]
        #    i += 1
    print(long_ans)

    print("Short answers: ")
    short_ans = []
    for i in range(len(nq_pred_dict[5225754983651766092]["short_answers"])):
        shorty = nq_pred_dict[5225754983651766092]["short_answers"][i]
        start_ttt = shorty["start_token"] 

        end_ttt = shorty["end_token"]
        ans = doc_toc[start_ttt:end_ttt]
        out_para= ''
        for a in ans:
    #print(ans)
             out_para += a["token"] + " "
        short_ans.append(out_para)
        print(out_para)
...
#estimator,run_config, model_fn, tokenizer = None,None,None,None

def main(_):
    print("Emoty main")


def write_eval_docs(url, doc_title):
    res = requests.get(url)
    html_page = res.content
    print("url is",url)
    soup = BeautifulSoup(html_page, 'html.parser')
...
def handle_my_custom_event(json, methods=['GET', 'POST']):
    ...
    global tokenizer
    global model_read
    global eval_filename
    global token_tuple_list1
    global la_cndidates1
    global url1
    global doc_title1
    global output1
    global read_file
    if json["question"] == 'User connected':
        ...
        if readSource == 0:
            if json['source_type'] == 'link':
                ...
                url_to_read = json['source']
                print("Reaidnf model from inside source")
                token_tuple_list1, la_cndidates1, url1, doc_title1, output1 = get_tokinezed_input(url_to_read)
                model_read = 1
            elif json['source_type'] == 'text':
                text = json['source']
                token_tuple_list1, la_cndidates1, url1, doc_title1, output1 = get_tokinezed_input_from_txt(text)
        ...
        #print("json source is ", json['source_type'])
        question_text = json['question']
        print('readsource is',readSource)
        print("text is", question_text)
        print("model read", model_read)
        #result = evaluate(model, tokenizer, prefix=1,text=text, question_text=question_text)
        print("get ans called")
        question, q_token_list = get_tokens_from_question(question_text)
        #j = get_input_json(token_tuple_list1, la_cndidates1, url1, doc_title1, output1, question, q_token_list)
        np_predictions_list = []
        max_prediction = -100000
        max_predict_doc = -1
        max_np_prediction = {}
        max_j = {}
        doc_files = ['doc1.txt', 'doc2.txt','doc3.txt', 'doc4.txt']
        #doc_files = ['doc1.txt', 'doc2.txt']
        #url_list = ["https://en.wikipedia.org//w/index.php?title=Subcutaneous_injection&amp;oldid=827949994_-7900537827363346667",
                       # "https://en.wikipedia.org//w/index.php?title=Human_skin&amp;oldid=837345516"]
        #for i in range(2):
        #    write_eval_docs(url_list[i], doc_files[i])
        ...
        if read_file == 1:
            for doc in doc_files:
                ...
                token_tuple_list1, la_cndidates1, url1, doc_title1, output1 = get_tokinezed_input_from_txt(text)
                j = get_input_json(token_tuple_list1, la_cndidates1, url1, doc_title1, output1, question, q_token_list)
                np_predictions = renamed_main()
                np_predictions_list.append(np_predictions)
                if np_predictions[5225754983651766092]['long_answer_score'] > max_prediction :
                    max_prediction = np_predictions[5225754983651766092]['long_answer_score']
                    max_predict_doc = doc
                    max_np_prediction = np_predictions
                    max_j = j
            print("Correct document is ", max_predict_doc)
            print("Max score is ", max_prediction)
            print("Actual answers are ")
            for n in np_predictions_list:
                print(n)
            long_ans, shoert_anwers = get_answer_text(max_np_prediction, max_j)
            long_ans = "Correct document is " +  max_predict_doc + ". " + long_ans
        
        else:
            j = get_input_json(token_tuple_list1, la_cndidates1, url1, doc_title1, output1, question, q_token_list)
            np_predictions = renamed_main()
            long_ans, shoert_anwers = get_answer_text(np_predictions, j)
        ans = "Long answer: " + long_ans
        ans += "Short answer : " + shoert_anwers[0]
        result = "Long answer: " + long_ans +  "Short answer : " + shoert_anwers[0]
        print("XXXXXX result is CXXXXX",result)
        json["answer"] = result
        print(json)
        print('ans appended')
        socketio.emit('my response', json, callback=messageReceived)
...
if __name__ == "__main__":
  ...
  #load_and_run_model()
  print('model loaded')
  socketio.run(app,debug=False, host='172.16.2.138')
  tf.app.run()
